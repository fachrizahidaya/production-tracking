...
class _SizingDetailScreenState extends State<SizingDetailScreen> {
  ...
  Future<void> _fetchDetail() async {
    ...
    try {
      final sizingService = Provider.of<SizingService>(context, listen: false);

      await sizingService.getDataView(context, widget.id);

      final detail = _detailData(sizingService.dataView);
  ...
  Future _handleDelete(dynamic _) async {
    ...
    showConfirmationDialog(
      context: context,
      title: 'Hapus Data',
      message: 'Apakah Anda yakin ingin menghapus proses sizing?',
      isLoading: _deleteLoading,
      buttonBackground: CustomTheme().buttonColor('danger'),
      onConfirm: () async {
        try {
          final message =
              await Provider.of<SizingService>(context, listen: false)
                  .deleteItem(
            context,
            widget.id.toString(),
            _deleteLoading,
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Lihat ringkasan proses Sizing dan item Persiapan Produksi.',
                    style: TextStyle(
...
class _ResultSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        _InfoRow('Panjang Gulungan', _withUnit(data['roll_length'], 'M')),
